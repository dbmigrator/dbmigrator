...
use crate::ddl::{DdlConfig, PgDdlRule};
    ...
    match cli.command {
        ...
        Some(Command::DumpDDL(args)) => {
            if let Some(db_url) = cli.db_url {
                ...
                let postgres_ddl_ruleset: Vec<PgDdlRule> = serde_yaml::from_str(include_str!("../ddlconfig.yaml")).map_err(|e| CliError::IoError(std::io::Error::new(std::io::ErrorKind::InvalidData, e)))?;
                let mut ddl_config: DdlConfig = DdlConfig::new();
                ddl_config.push_postgres_ddl_ruleset(postgres_ddl_ruleset);
                ...
                match Archive::parse(&mut file) {
                    Ok(archive) => {
                        ...
                        for entry in archive.toc_entries {
                            ...
                            let e = sql_files.entry(filename.clone()).or_insert("-- Auto-generated by dbmigrator. DO NOT EDIT!\n".to_string());
                            e.push_str(format!("-- Name: {}; Type: {}; Schema: {}; Owner: {}\n", entry.tag, entry.desc, if entry.namespace.is_empty() { "-" } else { &entry.namespace }, entry.owner).as_str());
